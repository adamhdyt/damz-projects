...
import { ArrowRight, Sparkles, Database, Zap, Globe, FileText, Mail } from "lucide-react"
...
/**
 * Desktop Scroll-Linked Showcase
 * This component is only rendered when mounted on desktop viewports.
 * The `containerRef` is unconditionally attached to its root `<div ref={containerRef}>`,
 * preventing the Framer Motion "Target ref is defined but not hydrated" error.
 */
function DesktopScrollShowcase() {
  ...
  // Phase 1 (0 to 0.35): Hero copy fades out
  ...
  // Phase 2 (0.35 to 0.6): Selected Work heading fades in
...
/**
 * Mobile & Reduced Motion Showcase
 * Standard clean responsive vertical layout without useScroll tracking.
 */
function MobileScrollShowcase() {
  return (
...
/**
 * Root ScrollShowcase Component
 * Safely guards against SSR hydration mismatch and ensures useScroll
 * is never initialized with an unhydrated ref.
 */
export function ScrollShowcase() {
  ...
  return (
    <div id="projects" className="relative w-full">
      {mounted && isDesktop && !shouldReduceMotion ? (
        <DesktopScrollShowcase />
      ) : (
        <MobileScrollShowcase />
      )}
    </div>
  )
}
