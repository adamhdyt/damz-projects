...
- [ ] `app/(blog)/layout.tsx` exists and renders the Sidebar (desktop) + MobileNav (mobile) + `{children}` in a responsive grid
- [ ] `app/(blog)/page.tsx` renders the Home/About view
- [ ] `app/(blog)/tech/page.tsx` renders a placeholder Tech Notes list
- [ ] `app/(blog)/tech/[slug]/page.tsx` renders a placeholder article detail
- [ ] `app/(blog)/life/page.tsx` renders a placeholder Life list
- [ ] `app/(blog)/life/[slug]/page.tsx` renders a placeholder article detail
- [ ] `Sidebar` component uses `<Link href="/tech">` etc. instead of `onSelect` callback
- [ ] `Sidebar` active state is derived from `usePathname()`, not from props
- [ ] `MobileNav` component uses `<Link>` + `usePathname()` instead of `onSelect` callback
- [ ] Browser back/forward buttons navigate correctly between all routes
- [ ] `ThemeToggle` is present and functional in both Sidebar and MobileNav across all routes
- [ ] Theme preference (dark/light) persists across route navigations and page reloads
- [ ] Old SPA-style `app/page.tsx` with `useState` is removed
